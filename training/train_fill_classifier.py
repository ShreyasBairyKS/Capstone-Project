"""
training/train_fill_classifier.py
==================================
MobileNetV3-Small classifier for bottle fill level: underfill / normal / overfill.

Stage 3 of the hybrid inspection pipeline.

Pipeline:
    Current YOLO → detects bottle bbox
    → crop bottle → this classifier → underfill / normal / overfill

Key features:
    • Focal Loss (gamma=2.0) with balanced class weights
    • Aspect-ratio-preserving letterbox resize (no distortion)
    • Class-weighted sampling for imbalanced datasets
    • Augmentation: brightness, contrast, flip, rotation, color jitter
    • MC Dropout for uncertainty quantification
    • Auto train/val split if only flat folders provided

Usage:
    python training/train_fill_classifier.py \
        --data-root data/fill_level \
        --auto-split \
        --epochs 50 \
        --batch 64 \
        --device 0 \
...
Expected folder structure (with --auto-split):
    data/fill_level/
        underfill/   *.jpg
        normal/      *.jpg
        overfill/    *.jpg
...
CLASS_NAMES = ["underfill", "normal", "overfill"]
NUM_CLASSES = 3
...
def main():
    ...
    parser.add_argument("--data-root", default="data/fill_level",
                        help="Root folder with underfill/ normal/ overfill/ subfolders")
    ...
    parser.add_argument("--run-name", default="fill_classifier_v1",
    ...
    print(f"\n{'='*60}")
    print("  Hybrid Pipeline — Stage 3: Fill Level Classifier")
    print(f"{'='*60}")
    ...
    # Loss — Focal Loss with balanced class weights
    # [underfill, normal, overfill] — balanced since all classes matter equally
    criterion = FocalLoss(
    ...
    final_path = models_dir / "fill_classifier_best.pth"
    ...
    context = {
        ...
        "notes": "Fill level classifier: underfill / normal / overfill",
    }
    ...
    if args.export and best_weights is not None:
        ...
        try:
            ...
            onnx_path = models_dir / "fill_classifier_best.onnx"
            torch.onnx.export(
    ...
    print(f"\n  → Next: integrate fill classifier into yoloWithFillLevel.py\n")
